    ...
    public function extract($filename, $originalFrames = false)
    {
        ...
        for ($i = 0; $i < count($this->frameSources); $i++) {
            ...
            if (!$originalFrames) {
                ...
                $framewidth=imagesx($img);
                $frameheight=imagesy($img);                     
                imagecopyresampled($sprite, $img, $this->frameSources[$i]["offset_left"], $this->frameSources[$i]["offset_top"], 0, 0, $framewidth, $frameheight, $framewidth, $frameheight);
                $img = $sprite;
            }
